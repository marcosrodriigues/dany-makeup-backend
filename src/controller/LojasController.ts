...
class LojasController {
    async index (request: Request, response: Response) {
        const { 
         ...
         try {
            if (filter === true) {
                const { stores, count } = await service.findAll(String(name), Number(limit), offset);
            ...
            const stores = await service.findWithoutFilter();
            return response.json(stores);
         } catch (err) {
    ...
    async show (request: Request, response: Response) {
        ...
        if (!id) return response.status(400).json({ message: 'No store provided' })
    ...
    async store (request: Request, response: Response) {
        ...
        const image_url = fileService.serializeImageUrl(file.filename, 'lojas');
